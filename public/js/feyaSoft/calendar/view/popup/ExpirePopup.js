 ...
 */
Ext.define('Ext.ux.calendar.popup.ExpirePopup', {
	extend : 'Ext.Window',
	initComponent : function() {
		...
		this.tpl = new Ext.XTemplate('<tpl for=".">'
						...
						+ lan['tpl.leftTime']
						+ ': <b>{left}</b></td>'
						+ '<td style="font-size:11px;"><a eid="{eventId}" class="x-calendar-edit-event" href="#">'
						+ lan['editEvent'] + '</a></td>'
						+ '</tr></tbody></table></div>' + '</div>' + '</tpl>');
		this.tpl.compile();

		this.viewpanel = Ext.create('Ext.Container',{
					region : 'center',
					autoScroll : true
				});

		this.hideCB = Ext.create('Ext.form.Checkbox',{ 
		...
		if (this.activedEvents) {
			...
			if (len == this.activedEvents.length && 0 != len) {
				for (var i = 0; i < len; i++) {
					...
					if (!(e.eventId == a.eventId && e.day == a.day
							&& e.eday == a.eday && e.startRow == a.startRow && e.endRow == a.endRow)) {
						return true;
					}
				}
			} else {
