...
class CalendarAgent{
	 ...
	 */
    function CalendarAgent($host="",$user="",$passwd="",$name=""){        
	 ...
	 *@param $host:host of mysql, $user: username of mysql, $passwd: password of mysql, $name: the db you want connect
	 */
    function connect($host="",$user="",$passwd="",$name=""){
        if($host=="") $host=$this->dbHost;
        if($user=="") $user=$this->dbUser;
        if($passwd=="") $passwd=$this->dbPwd;
    ...
    function showAllCalendar($params){
    ...
    function showOnlyCalendar($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback        
        $now = date('Y-m-d H:i:s');
        $id = $params['id'];
        if($id){
    ...
    function createUpdateCalendar($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback        
        $now = date('Y-m-d H:i:s');
        $id = $params['id'];
        $flag = 0;
    ...
    function deleteEventsByCalendar($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback
    ...
    function loadEvent($params){
        ...
        if(0 < mysql_num_rows($rs)){
            while($obj = mysql_fetch_object($rs)) {                
                ...
                $obj->{'eymd'} = $pies[0];
                $obj->{'endTime'} = $pies[1];
                $obj->{'alertFlag'} = $this->getEventReminders($obj->{'id'});
                $data[] = $obj;
            }
        }
        $arr = array();
            ...
            if(!is_null($alertFlag)){
                ...
                try{                    
                    for($i = 0, $len = count($alertJson); $i < $len; $i++){
                        ...
                        $rid = $this->createEventReminder($bid, $alertObj['type'], $alertObj['early'], $alertObj['unit']);
                        $eventReminder = $this->getReminderWithEvent($rid);                        
                        $info = $this->checkEmailReminder($eventReminder, true);
                    }
                }catch(Exception $e){
    ...
    function changeDay($params){
        ...
        while($obj = mysql_fetch_object($rs)) {            
            ...
            if("true" == $keep){
                ...
                $nid = mysql_insert_id();
                $backids->{$oid} = $nid;
                $this->copyEventReminders($nid, $oid);
            }else{
    ...
    function getSetting($params){
        ...
        if(0 == mysql_num_rows($rs)){
            ...
            $query = "INSERT INTO ".$this->dbPrefix."calendar_setting (userId, hourFormat, dayFormat, weekFormat, monthFormat, fromtoFormat, language, activeStartTime, activeEndTime, intervalSlot, createByDblclick, hideInactiveRow, singleDay, startDay, readOnly, initialView)"
                ."VALUES (".$userId.", '24', 'l M d Y', 'm/d(D)', 'm/d', 'm/d/Y', 'en', '00:00', '24:00', '30', 0, 0, 0, 1, 0, 1)";
            $rs = mysql_query($query);
            $query = "SELECT * FROM ".$this->dbPrefix."calendar_setting WHERE userId = '".$userId."'";
            $rs = mysql_query($query);
        }
	...
	function loadSetting($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback
        $data = $this->getSetting($params);
        $arr = array();
		$arr['total'] = count($data);
		$arr['data'] = $data;
        return $this->backJson($arr, $callback);
    ...
    function getRepeatEvent($params){
        $userId = $params['userId'];
        $query = "SELECT CE.*, CT.color FROM ".$this->dbPrefix."calendar_event AS CE, ".$this->dbPrefix."calendar_type AS CT "
                ."WHERE CT.id = CE.calendarId "
                ."AND CE.userId = ".$userId." "
    ...
    function loadRepeatEvent($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback
    ...
    function createUpdateRepeatEvent($params){
        ...
        $insertSQL = "INSERT INTO ".$this->dbPrefix."calendar_event (userId, calendarId, creation_date, startTime, endTime, description, subject, update_date, locked, repeatType) "
            ...
            ."', '".$params['description']."', '".$params['subject']."', '".$now."', ".$locked.", '".$params['repeatType']."')";
        if($id){
            $oflag = $this->isRepeatEvent($params['oldRepeatType']);
            $flag = $this->isRepeatEvent($params['repeatType']);
            if($oflag && $flag){
                $query = "UPDATE ".$this->dbPrefix."calendar_event SET calendarId = ".$params['calendarId'].", description = '".$params['description']
                    ."', startTime = '".$params['startDay']." ".$params['startHMTime']."', endTime = '".$params['endDay']." ".$params['endHMTime']."', "
                    ."locked = ".$locked.", subject = '".$params['subject']."', update_date = '".$now."', "."repeatType = '".$params['repeatType']."' "
        ...
        if($rs){
            ...
            if(!is_null($alertFlag)){
                ...
                try{
                    for($i = 0, $len = count($alertJson); $i < $len; $i++){
                        $alertObj = $alertJson[$i];
                        $rid = $this->createEventReminder($id, $alertObj['type'], $alertObj['early'], $alertObj['unit']);
                        $eventReminder = $this->getReminderWithEvent($rid);
                        $info = $this->checkEmailReminder($eventReminder, true);
                    }
                }catch(Exception $e){
    ...
    function deleteRepeatEvent($params){
        $callback = isset( $params['callback'] ) ? $params['callback'] : null; // Undefined index: callback
        $id = $params['id'];
    ...
    function isRepeatEvent($repeatType){
    ...
    function backJson($arr, $callback = null){
        $total = $arr['total'];
    ...
    function wrapCallback($str, $callback){
        if(null != $callback){
            return $callback.'('.$str.');';
        }
		return $str;
    ...
    function deleteEventReminder($eventId){
        $query = "DELETE FROM ".$this->dbPrefix."calendar_event_reminder WHERE eventId = ".$eventId;
        mysql_query($query);
    }

    function createEventReminder($eventId, $alertType, $alertEarly, $timeUnit){
    ...
    function getEventReminders($eventId){
    ...
    function copyEventReminders($newId, $oldId){
        $query = "SELECT * FROM ".$this->dbPrefix."calendar_event_reminder WHERE eventId = ".$oldId;
        $rs = mysql_query($query);
        if(0 < mysql_num_rows($rs)){
            while($obj = mysql_fetch_object($rs)) {
                $this->createEventReminder($nid, $obj->{'type'}, $obj->{'early'}, $obj->{'unit'});
            }
        }
    }

    function saveEventReminder($params){
        $arr = array();
    ...
    function getReminderWithEvent($id){
    ...
    function SendReminderEmail(){        
        ...
        if(0 < mysql_num_rows($rs)){
            while($obj = mysql_fetch_object($rs)) {
                $this->checkEmailReminder($obj, false);
            }
        }
    }

    function checkEmailReminder($eventReminder, $checkAll){
        $info;
        ...
        if('no' == $repeatType || 'exception' == $repeatType){            
            if(!is_null($alerted)){
                if($endTime >= $nowTime){
                    ...
                    if($startTime <= $limitTime){
                        $info = $this->sendEmail($eventReminder);
                    }
                }
                $this->saveEventReminder(array('alerted'=>'forever', 'id'=>$eventReminder->{'id'}));
            }
            ...
            if(true == $checkAll){
                ...
                for($i = $dspan; $i >= 0; $i--){
                    ...
                    $ret = $this->checkEmailReminderOfRepeatEvent($earlyTime, $nowday, $limitTime, $repeatJson, $eventReminder);
                    if(!is_null($ret)) $info = $ret;
                }
            }else{
                $info = $this->checkEmailReminderOfRepeatEvent($earlyTime, $nowday, $limitTime, $repeatJson, $eventReminder);
            }
        }
        return $info;
    }

    function checkEmailReminderOfRepeatEvent($earlyTime, $nowDay, $limitTime, $repeatJson, $eventReminder){
        $startTime = $eventReminder->{'startTime'};
        $endTime = $eventReminder->{'endTime'};
        $dspan = $repeatJson['dspan'];
        ...
        if($beginTime <= $limitTime){
            ...
            if($flag){
                ...
                if(!is_null($epts)){
                    if(!is_null($epts[limitDay])){
                        $this->saveEventReminder(array('alerted'=>$nowday, 'id'=>$eventReminder->{'id'}));
                        return;
                    }
                }
            }
            ...
            if($match){
                ...
                if($startHM <= $limitHM){/*if start hour and minute is earlier than limit hour minute, it means we should send email to alert user*/
                    ...
                    $this->saveEventReminder(array('alerted'=>$nowday, 'id'=>$eventReminder->{'id'}));
                    return $this->sendEmail($eventReminder);
                }
            }else{
                $this->saveEventReminder(array('alerted'=>$nowday, 'id'=>$eventReminder->{'id'}));
            }
     ...
     */
    function sendEmail($eventReminder){        
